<?php
	...
	function add_user_model($username,$password,$email,$hoten,$diachi,$phone,$authen_key)
	{
		...
		$role=1;
		$status=0;
		$sql="INSERT INTO taikhoan(TenDangNhap,MatKhau,TenHienThi,DiaChi,SDT,Email,Quyen,Trang_thai,authen_key,create_time,update_time) VALUES(:username,:password,:hoten,:diachi,:phone,:email,:quyen,:status,:authen_key,:create_time,:update_time)";
	...
	function update_user($id_decode)
	{
		...
		$conn=connection();
		$active=1;
		$sql="UPDATE taikhoan AS a SET a.Trang_thai=:active WHERE a.id_tk= :id";
		$stmt=$conn->prepare($sql);
		if($stmt)
		{
